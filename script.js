...
let xPlayer = 50,
    ...
    xEnemy = 750,
    yEnemy = 680,
    boardPicPlayer, boardPicEnemy, boardPicCoin, countLife = 3,
    countCoin = 0;
...
function draw() {
    ...
    if (!startGame) {
        ...
        function drawButton(strokeColor, fillColor) {
            ...
            drawPath(buttonStart, strokeColor, fillColor, 5);
            let buttonText = 'Start';
            printText('Start', xButton+widthButton/2-innerWidth*0.01, yButton+heightButton/2, 25, "Black");
        }
       
        canvas.addEventListener('mousemove',(event)=>{
            if(ctx.isPointInPath(buttonStart,event.clientX,event.clientY)){
                drawButton('SkyBlue','RoyalBlue')
            }else{
 drawButton('DarkBlue', 'Blue');
        ...
        function moveEnemy() {
            function collisionEnemy() {
                if (checkCollision(xPlayer, xEnemy, yPlayer, yEnemy, boardPicPlayer[0], boardPicEnemy[0], boardPicPlayer[1], boardPicEnemy[1])) {
                    countLife--;
                    if (countLife <= 0) {
                        let newGame = confirm('ТЫ ЛОХ!!!!!! ТЫ слил 1000000000000000000БИТКОИНОВ!!!!!!!!');
                        if (newGame) {
                            countLife = 3;
                        } else {
        ...
        collisionCoin();
        printText("Count = " + countCoin, innerWidth * 0.05, innerHeight * 0.05 + picLife.height, 15, "Green");
